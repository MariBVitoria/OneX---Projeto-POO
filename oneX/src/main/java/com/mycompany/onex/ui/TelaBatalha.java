 ...
 * Obs: A maioria das declarações estão no fim do arquivo
 */
public class TelaBatalha extends javax.swing.JFrame {
    ...
    public TelaBatalha(Batalha batalha) {
        ...
        initComponents(); // Metodo gerado pelo NetBeans para desenhar a tela
    ...
    private void configurarBatalhaInicial() {
        this.setTitle("Batalha" + batalha.getJogador1().getNome() + " vs " + batalha.getCpu().getNome());
        
        // Nomes
        labelNomeJogador.setText(batalha.getJogador1().getNome());
        labelNomeComputador.setText(batalha.getCpu().getNome());
        ...
        logBatalha.append("A batalha começou! " + batalha.getJogador1().getNome() + " enfrenta " + batalha.getCpu().getNome() + ".\n");
    }
    ...
    private String verificarFimDeJogo() {
        // 1. Pergunta ao "Cérebro" se o jogo acabou
        String resultado = batalha.verificarFimDeJogo();

        // 2. Se o 'resultado' NÃO for nulo, o jogo acabou!
        if (resultado != null) {
            adicionarLog("------------------------------------");
            adicionarLog(resultado.toUpperCase());

            // 3. Desabilita os botões inúteis
            botaoHabilidade.setEnabled(false);
            botaoUsarItem.setEnabled(false);
            comboInventario.setEnabled(false);

            // 4. === A MÁGICA (REAPROVEITANDO O BOTÃO) ===

            // Deixa o botão de atacar clicável de novo
            botaoAtacar.setEnabled(true);

            // Muda o texto do botão
            botaoAtacar.setText("Jogar de Novo");

            // 5. Remove a ação antiga (de atacar)
            // (Isso é importante para ele não tentar atacar e reiniciar ao mesmo tempo)
            for (java.awt.event.ActionListener al : botaoAtacar.getActionListeners()) {
                botaoAtacar.removeActionListener(al);
            }

            // 6. Adiciona a AÇÃO NOVA (de reiniciar)
            botaoAtacar.addActionListener(new java.awt.event.ActionListener() {
                public void actionPerformed(java.awt.event.ActionEvent evt) {
                    // 1. Fecha a janela de batalha
                    dispose();

                    // 2. Chama o Menu de Seleção DE NOVO
